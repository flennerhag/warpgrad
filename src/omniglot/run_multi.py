...
parser = argparse.ArgumentParser(
    description='Multi-Omniglot experiment')
parser.add_argument(
    '--model', required=True, type=str, help='Meta model to run')
parser.add_argument(
    '--niter', default=10, type=int, help='(default=%(default)d)')
parser.add_argument(
    '--ngpus', default=1, type=int, help='(default=%(default)d)')
parser.add_argument(
    '--npres', default=[1, 3, 5, 10, 15, 20, 25],
    type=int, nargs='+', help='(default=%(default)r)')
parser.add_argument(
    '--npar', default=2, type=int, help='(default=%(default)d)')
parser.add_argument(
    '--sleep', default=120, type=float, help='(default=%(default)f)')
parser.add_argument(
    '--seed', default=6534, type=int, help='(default=%(default)d)')
...
def gen_job(npre, inner_lr, outer_lr, train_steps, model, gpx, seed):
    ...
    call = "python main.py \
            ...
            --device {} \
            --meta_model {} \
            --seed {}".format(npre, train_steps,
                              inner_lr, outer_lr,
                              jobname, gpx, model, seed)
    return call
    ...
    for s in seeds:
        for p in args.npres:
            ...
            while not case_running:
                ...
                if available:
                    gpx = available.pop(0)
                    call = gen_job(p, inner_lr, outer_lr,
                                   train_steps, args.model, gpx, s)
                    res = pool.apply_async(os.system, [call])
                    results.append((res, gpx))
                    case_running = True
                else:
                    time.sleep(args.sleep)
            print('Launched {} on {} with seed {} (complete: {}/{})'.format(
